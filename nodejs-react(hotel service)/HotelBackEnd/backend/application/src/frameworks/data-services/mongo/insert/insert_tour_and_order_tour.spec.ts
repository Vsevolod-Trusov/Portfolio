...
describe('Insert room and room type', () => {
  ...
  it('should insert meal', async () => {
    ...
    tour.tourName = 'park'
    tour.description = 'cool park'
    tour.tourDate = new Date()
    tour.tourPrice = 5.25

    await service.tours.create(<TourDocument>tour)
    let val = true
    expect(val).toBe(true);
  });

  it('should insert orderTour', async () => {
    service.onApplicationBootstrap()

    let tour: Tour = await service.tours.get('6416f5920aa09a4a6b6c9e10')
    let user: User = await service.users.get('6416df17ec599def02cfdc0b')
    let orderTour: OrderTour = new OrderTour()

    orderTour.tourName = tour
    orderTour.peopleAmount = 3
    orderTour.orderPrice = 3 * tour.tourPrice
    orderTour.userLogin = user

    await service.orderTours.create(<OrderTourDocument>orderTour)
    let val = true
    expect(val).toBe(true);
  });
});
