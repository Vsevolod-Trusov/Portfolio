...
#define ERROR_THROW_IN(id,  l,c) Error::geterrorin(id, l, c);		// throw ERROR_THROW_IN(id, строка, столбец)
#define ERROR_THROW_L(id,l,c) Error::geterrors(id,l,c);					
...
//ERROR_ENTRY_NODEF10(id) - 10 неопределенный элемент таблицы ошибок
...
namespace Error
{
	struct ERROR	// тип исключения для throw ERROW_THROW |ERROR_THROW_IN и catch(ERROR)
	{
		int id;									// код ошибки
